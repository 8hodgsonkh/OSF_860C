...
__RAM_FUNC void calculate_id_part1(){  // to be called in begin of ISR 1 when rotor position has been updated and current are measured
    ...
    __RAM_FUNC void calculate_id_part2(){ // to be called at the end of ISR1 (so cordic has time to finish)
...
// Average-based filtered battery current (15b -> 10b), with safety clamp
if (ui32_adc_battery_current_15b_moving_average > (255U << 5)) { // clamp for safety
    ui32_adc_battery_current_15b_moving_average = 255U << 5;
}
// from 15 bits to 10 bits like TSDZ2
ui16_adc_battery_current_filtered = (uint16_t)(ui32_adc_battery_current_15b_moving_average >> 5);

// If you ever want per-rotation averaging instead of moving average, the old line was:
// ui16_adc_battery_current_filtered = (uint16_t)(ui32_adc_battery_current_1_rotation_15b >> 5);
...
//    #if (APPLY_ENHANCED_POSITIONING == 0)
    ...
    // it is set on 66 based on tests with my motor. (note : 64 = 90°)
    ...
    // just to test without foc_angle
