...
extern uint8_t ui8_smooth_start_enabled;
...
//    uint8_t new_hall_pattern = 0; // filled only when a new pattern is detected
    ...
    // Set torque target for FOC controller from application code
    // This is a placeholder, you'll need to feed the real target from your ebike_app logic
    // For now, let's tie it to the duty cycle for testing.
    foc_controller.Iq_target_q15 = FLOAT_TO_Q15((float)ui8_g_duty_cycle / 255.0f);
    foc_controller.Id_target_q15 = 0; // Standard FOC, no field weakening for now

    foc_run_control_loop(&foc_controller);

    // Use the PWM values calculated by the FOC controller
    ui16_a = foc_controller.pwm_u;
    ui16_b = foc_controller.pwm_v;
    ui16_c = foc_controller.pwm_w;
...
//    #endif
    ...
    if (irq0_min > temp) irq0_min = temp; // store the in enlased time in the irq
    if (irq0_max < temp) irq0_max = temp; // store the max enlased time in the irq
    #endif
...
//void CCU80_1_IRQHandler(){ // called when ccu8 Slice 3 reaches 840  counting DOWN (= 1/4 of 19mhz cycles)    
    ...
    // a valid time occurred : save the counter with the elapsed time * 55usec
