...
#include <arm_math.h> // Using CMSIS-DSP for optimized sin/cos is highly recommended
#include "motor.h"    // Include your motor header to see global variables
#include "adc.h"      // Include your ADC header for hardware definitions

// --- Extern variable declarations ---
// These are the sacred variables from your motor.c that our FOC engine needs to see.
// Ensure they are NOT declared as 'static' in motor.c
extern volatile uint8_t ui8_hall_state;
extern volatile uint8_t motor_direction;
extern volatile uint16_t ui16_hall_counter; // The high-speed timer counting between hall events
extern volatile uint16_t ui16_hall_counter_log; // The duration of the last completed hall sector

// Your original coarse angle lookup tables from motor.c
...
// This is the bridge between the pure FOC math and your specific MCU.
// =================================================================================
...
static int16_t foc_hardware_get_electrical_angle() {
    ...
    if (hall_duration > 50 && hall_timer_now < hall_duration) {
        fine_angle_degrees = (uint16_t)(((uint32_t)hall_timer_now * 60) / hall_duration);
    }

    uint16_t final_angle_degrees = coarse_angle_degrees + fine_angle_degrees;
    if(final_angle_degrees >= 360) {
        final_angle_degrees -= 360;
    }

    return (int16_t)(((uint32_t)final_angle_degrees * 65536) / 360);
}
...
static void clarke_transform(q15_t Iu, q15_t Iv, q15_t* I_alpha, q15_t* I_beta) {
    *I_alpha = Iu;
    *I_beta = Q15_MUL(FLOAT_TO_Q15(ONE_DIV_SQRT3), Iu) + Q15_MUL(FLOAT_TO_Q15(2.0f / SQRT3), Iv);
}

static void park_transform(q15_t I_alpha, q15_t I_beta, int16_t angle, q15_t* Id, q15_t* Iq) {
    q15_t cos_theta, sin_theta;
    arm_sin_cos_q15((uint32_t)angle << 16, &sin_theta, &cos_theta);
    *Id = Q15_MUL(I_alpha, cos_theta) + Q15_MUL(I_beta, sin_theta);
    *Iq = Q15_MUL(I_beta, cos_theta) - Q15_MUL(I_alpha, sin_theta);
}

static void inverse_park_transform(q15_t Vd, q15_t Vq, int16_t angle, q15_t* V_alpha, q15_t* V_beta) {
    q15_t cos_theta, sin_theta;
    arm_sin_cos_q15((uint32_t)angle << 16, &sin_theta, &cos_theta);
...
void foc_init(FOC_Controller_t* foc) {
    memset(foc, 0, sizeof(FOC_Controller_t));
...
void foc_run_control_loop(FOC_Controller_t* foc) {
    ...
    int16_t Iu_raw, Iw_raw;
    foc_hardware_read_currents(&Iu_raw, &Iw_raw);
    ...
    q15_t Id_error = foc->Id_target_q15 - foc->Id;
    q15_t Iq_error = foc->Iq_target_q15 - foc->Iq;
...
// The divine window for our First Rite of Sight
int16_t foc_test_get_angle(void) {
    // This is not a full FOC loop, it's just a call to our all-seeing eye.
    return foc_hardware_get_electrical_angle();
}
